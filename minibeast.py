...
def vul_info(val):
	...
	if val == 'c':
		...
		result = bcolors.BG_HIGH_TXT+" major "+bcolors.ENDC
	elif val == 'm':
		result = bcolors.BG_MED_TXT+" moderate "+bcolors.ENDC
	elif val == 'l':
		result = bcolors.BG_LOW_TXT+" minor "+bcolors.ENDC
	else:
		result = bcolors.BG_INFO_TXT+" cosmetic "+bcolors.ENDC
	return result
...
# Minibeast Help Context
def helper():
        print bcolors.OKBLUE+"Information:"+bcolors.ENDC
        print "------------"
        print "\t./minibeast.py example.com: Scans the domain example.com"
        print "\t./minibeast.py --help     : Displays this help context."
        print bcolors.OKBLUE+"Interactive:"+bcolors.ENDC
        print "------------"
        print "\tCtrl+C: Skips current test."
        print "\tCtrl+Z: Quits minibeast."
        ...
        print "--------------------------"
        print "\t"+vul_info('c')+": A core functionality of the system fails or the system doesn’t work at all."
        print "\t"+vul_info('h')+"    : The defect impacts basic functionality and the system is unable to function properly."
        print "\t"+vul_info('m')+"  : The defect causes the system to generate false, inconsistent, or incomplete results."
        print "\t"+vul_info('l')+"     : The defect impacts the business but only in very few cases."
        print "\t"+vul_info('i')+"    : The defect is only related to the interface and appearance of the application.\n"
...
class Spinner:
    ...
    def spinner_task(self):
        try:
            ...
            #clear()
            print "\n\t"+ bcolors.BG_ERR_TXT+"Minibeast received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC
            sys.exit(1)
...
# Tool Responses (Begins) [Responses + Severity (c - critical | h - major | m - medium | l - low | i - informational) + Reference for Vuln Definition and Remediation]
tool_resp   = [
...
tools_fix = [
					[1, "Sensitive Information Leakage Detected. The ASP.Net application does not filter out illegal characters in the URL. The attacker injects a special character (%7C~.aspx) to make the application spit sensitive information about the server stack.",
					...
					[3, "It is not bad to have a CMS in Drupal. There are chances that the version may contain vulnerabilities or any third party scripts associated with it may possess vulnerabilities",
					...
					[4, "It is not bad to have a CMS in Joomla. There are chances that the version may contain vulnerabilities or any third party scripts associated with it may possess vulnerabilities",
					...
					[5, "Sometimes robots.txt or sitemap.xml may contain rules such that certain links that are not supposed to be accessed/indexed by crawlers and search engines. Search engines may skip those links but attackers will be able to access it directly.",
							"It is a good practice not to include sensitive links in the robots or sitemap files."],
					[6, "Without a Web Application Firewall, An attacker may try to inject various attack patterns either manually or using automated scanners. An automated scanner may send hordes of attack vectors and patterns to validate an attack, there are also chances for the application to get DoS`ed (Denial of Service)",
					...
					[7, "Open Ports give attackers a hint to exploit the services. Attackers try to retrieve banner information through the ports and understand what type of service the host is running",
					...
					[9, "Zone Transfer reveals critical topological information about the target. The attacker will be able to query all records and will have more or less complete knowledge about your host.",
...
# Tool Set
tools_precheck = [
					["wapiti"], ["nmap"], ["golismero"], ["uniscan"], ["wafw00f"], ["dirb"], ["xsser"], ["dnsrecon"], ["sslyze"], ["davtest"], ["nikto"]
			     ]
    ...
    if target == '--help' or target == '-h' or target == '--h':
        ...
        while (rs_avail_tools < len(tools_precheck)):
			...
			try:
				...
				print "\t"+bcolors.BG_ERR_TXT+"Minibeast was terminated abruptly..."+bcolors.ENDC
				sys.exit(1)
			if "not found" in val:
        ...
        if unavail_tools == 0:
        	print "\t"+bcolors.OKGREEN+"All Scanning Tools are available. All vulnerability checks will be performed by Minibeast."+bcolors.ENDC
        else:
        	print "\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable. Minibeast can still perform tests by excluding these tools from the tests. Please install these tools to fully utilize the functionality of Minibeast."+bcolors.ENDC
        ...
        while(tool < len(tool_names)):
            print "["+tool_status[tool][arg3]+tool_status[tool][arg4]+"] Minibeast(Assault) "+str(tool+1)+"/"+str(tool_checks)+" | "+bcolors.OKBLUE+tool_names[tool][arg2]+bcolors.ENDC,
		...
		rs_skipped_checks = rs_skipped_checks + 1
            ...
            if runTest == 1:
                    ...
                    clear()
                    print "\t"+bcolors.WARNING + "Test Skipped. Performing Next. Press Ctrl+Z to Quit Minibeast." + bcolors.ENDC
                    rs_skipped_checks = rs_skipped_checks + 1
        ...
        if len(rs_vul_list)==0:
        	...
        	with open("RS-Vulnerability-Report", "a") as report:
        		while(rs_vul < len(rs_vul_list)):
	       		...
	       		print "\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+"`RS-Vulnerability-Report`"+bcolors.ENDC+" is available under the same directory Minibeast resides."
